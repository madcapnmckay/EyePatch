...
namespace EyePatch.Core.Mvc.Sitemap
{
    public static class XmlSiteMap
    {
        ...
        {
            var xDoc = new XDocument(new XDeclaration("1.0", HttpContext.Current.Response.ContentEncoding.WebName, "yes"),
        ...
        {
            return !date.HasValue ? string.Empty : string.Format("{0}-{1}-{2}T{3}:{4}:{5}+00:00", new object[] { date.Value.Year, date.Value.Month.ToString("00"), date.Value.Day.ToString("00"), date.Value.Hour.ToString("00"), date.Value.Minute.ToString("00"), date.Value.Second.ToString("00") });
        }
    }
}